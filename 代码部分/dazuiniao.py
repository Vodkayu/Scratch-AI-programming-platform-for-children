...
model_path = './trained_models/kapian.h5'
...
def lianxukapian(frame, kpcishu, wkpcishu):
    global no_num
    global cur_flag
    global kapian_arr
    global chuxian_num
    ...
    try:
        frame_c = frame.copy()
        frame_c, out, _, index, _ = kapianshibie(frame_c)
        # print(out, index)
        if out != ' ':
            # 如果0-10以及13-17为不包含=（）的数字卡片
            if (index in range(0,10)) or (index in range(13,17)):
                zhanshi2 = 3
                if no_num2 <= 0:
                    cur_flag2 = 1
                    chuxian_num2 = kpcishu
                    no_num2 = wkpcishu
                    kapian_cur2 = 'kapian'
                if chuxian_num2 == 0 and cur_flag2:
                    kapian_arr2.append(kapian_cur2)
                    # print(kapian_arr2)
                    cur_flag2 = 0
                if chuxian_num2 in range(1,kpcishu-2):
                    if out == kapian_cur2:
                        chuxian_num2 -= 1
                if chuxian_num2 in [kpcishu-2, kpcishu-1]:
                    if out == kapian_cur2:
                        chuxian_num2 -= 1
                    else:
                        chuxian_num2 = kpcishu
                if chuxian_num2 == kpcishu:
                    kapian_cur2 = out
                    chuxian_num2 -= 1
            if index in range(17,43):
                zhanshi = 3
                if no_num <= 0:
                    cur_flag = 1
                    chuxian_num = kpcishu
                ...
                if chuxian_num == kpcishu:
                    kapian_cur = out
                    chuxian_num -= 1
            # 输出单词部分，将下面掉头改为其他卡片，即可作为停止符号
            if out == '掉头' and kapian_arr != []:
                zhanshi -= 1
                if zhanshi == 0:
                    zhanshi = 3
                    # print('输出字符串结果：' + ''.join(kapian_arr))
                    kparr = ''.join(kapian_arr)
                    kapian_arr = []
                    return kparr, frame_c
            # 计算公式部分
            gongshi = ''.join(kapian_arr2)
            if out == '=' and kapian_arr2 != [] and gongshi != '':
                zhanshi2 -= 1
                if zhanshi2 == 0:
                    zhanshi2 = 3
                    for i in range(len(gongshi)):
                        if (gongshi[i] < '0') or (gongshi[i] > '9'):
                            fuhao.append(i)
                    try:
                        for i in range(len(fuhao)):
                            if i == 0:
                                jisuan = int(gongshi[0:fuhao[i]])
                            if i != (len(fuhao)-1):   
                                if gongshi[fuhao[i]] == '+':
                                    jisuan += int(gongshi[fuhao[i]+1:fuhao[i+1]])
                                if gongshi[fuhao[i]] == '-':
                                    jisuan -= int(gongshi[fuhao[i]+1:fuhao[i+1]])
                                if gongshi[fuhao[i]] == '*':
                                    jisuan *= int(gongshi[fuhao[i]+1:fuhao[i+1]])
                    ...
                    gongshi = ''
                    fuhao = []
                    gongshi_t = gongshi_t + '=' + str(jisuan_t)
                    return gongshi_t, frame_c
...
def detect_face(img):
    ...
    for i in range(len(faces)):
        if maxl < faces[i][2]*faces[i][3]:
            maxl = faces[i][2]*faces[i][3]
            face_index = i
    (x, y, w, h) = faces[face_index]
    return gray[y:y+h, x:x+w], faces
...
# def predictper(test_img):
#     img = test_img.copy()
#     face, faces = detect_face(img)
#     renyuan = []
# #     print(rect)
#     try:
#         for i in range(len(faces)):
...
#             if label[1] >= 40:
#                 label_text = str(labels[label[0]])
#                 renyuan.append(label_text)
#                 draw_rectangle(img, rect)
#     #         draw_text(img, label_text, rect[0], rect[1]-5)
#                 img = add_chinese(img,label_text,(rect[0], rect[1]-20))
#                 # cv2.imshow('out', img)
#                 return renyuan, img
#         return renyuan, img
#     except Exception as e:
# #         print(e)
#         # cv2.imshow('out', test_img)
#         return 0, test_img
...
# 人脸识别模块
faces, labels = prepare_training_data("picture")
print('已经录入人脸：')
print(labels)
...
face_recognizer.train(faces, np.array(label_index))
tracker = cv2.TrackerCSRT_create()

# 使用LBP跟踪算法
def genzong(frame, biaoding):
    global tracker
    if biaoding:
        tracker = cv2.TrackerCSRT_create()
    ...
    try:
        ...
        if ok:
            ...
            zx = int((bbox[0]*2 + bbox[2])/2)
            zy = int((bbox[1]*2 + bbox[3])/2)
            frame = add_chinese(frame,'({}, {})'.format(zx, zy),(0, 60))
            # cv2.imshow('out', frame)
            return zx, zy, frame
        else:
            return 0, 0, frame
