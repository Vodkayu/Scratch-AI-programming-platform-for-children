...
class Client():
    ...
    def FormLoad(self):
        ...
        self.ui.check_auto.clicked.connect(self.check_auto_click)
        self.ui.check_return.clicked.connect(self.chech_return_click)
    ...
    def process(self,frame):
        ...
        if self.process_mode==0:
            ...
            if zx and zy:
                ...
                self.count+=1
                if self.isReturn:sendResult('mbgz',str(zx),str(zy))
            # if not res:
            self.baoding=0
            ...
            if out:
                bli = '连续卡片:' + str(out)
                self.printMsg(bli)
                if self.isReturn:sendResult('lxkpsb', out)
            ...
            if zx!=0 and zy!=0:
                ...
                if self.isReturn:sendResult('kpgz', str(zx), str(zy))
                self.count+=1
        return res
    ...
    def chech_return_click(self):
        self.isReturn=self.ui.check_return.isChecked()
    def check_auto_click(self):
        # hint = '关闭自动模式' if self.isAuto else '打开自动模式'
        self.isAuto=self.ui.check_auto.isChecked()
